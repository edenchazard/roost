...
fn save_media_to_disk(file_path: &String, data: &Vec<u8>) {
    if let Err(e) = fs::write(&file_path, &data) {
        eprintln!("Error writing media file {}: {}", &file_path, e);
    }
}

fn pluck_media_data_from_tracks(file_path: &String) -> Result<Vec<Media>, ()> {
...
pub async fn sync_tracks(tracks_in: Vec<InputTrack>) {
    ...
    for insert in inserted_tracks.iter() {
        match &insert.audio_url {
            Some(path) => {
                let medias = pluck_media_data_from_tracks(path).unwrap();
                let front_cover = medias.iter().find(|m| m.media_type == "Front cover");

                populate_media_to_db(&medias, conn).unwrap();

                match front_cover {
                    Some(media_file) => {
                        save_media_to_disk(&media_file.path, &media_file.data);

                        update(schema::tracks::table)
                            .filter(schema::tracks::id.eq(insert.id))
                            .set(schema::tracks::picture_url.eq(Some(media_file.path.clone())))
                            .execute(conn)
                            .unwrap();
                    }
                    None => (),
        ...
        let albums = schema::albums::table
            .filter(schema::albums::id.eq_any(&album_ids))
            .load::<models::Album>(conn)
            .unwrap();

        for album in &albums {
            if album.picture_url.is_none() {
                let album_tracks = schema::tracks::table
                    .filter(schema::tracks::album.eq(album.title.clone()))
                    .filter(schema::tracks::album_artist.eq(album.artist.clone()))
                    .load::<models::Track>(conn)
                    .unwrap();

                for track in album_tracks {
                    if let Some(picture_url) = track.picture_url {
                        update(schema::albums::table)
                            .filter(schema::albums::id.eq(album.id))
                            .set(schema::albums::picture_url.eq(Some(picture_url)))
                            .execute(conn)
                            .unwrap();
                        break;
                    }
...
fn populate_media_to_db(medias: &Vec<Media>, conn: &mut PgConnection) -> Result<(), ()> {
